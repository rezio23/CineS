...
async function run() {
  ...
  try {
    ...
    // Movie_Show was cleared earlier; recreate three minimal shows for FK.
    ...
    const bookingSql = `INSERT INTO Booking (customer_id, show_id, staff_id, booking_date, payment_method, total_amount, status)
                        VALUES (:customer_id, :show_id, :staff_id, TO_DATE(:booking_date, 'YYYY-MM-DD HH24:MI:SS'), :payment_method, :total_amount, :status)`;
    ...
    for (const { year, month, count } of TARGETS) {
      for (let i = 0; i < count; i++) {
        ...
        const customerId = ((inserted) % 3) + 1;
        const staffId = ((inserted) % 3) + 1;
        const showId = ((inserted) % 3) + 1;
    ...
    console.log('Inserted', inserted, 'Booking rows');
  } finally {
